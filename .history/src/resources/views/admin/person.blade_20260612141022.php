...
<div class="content">
    <div class="content-item">
        ...
        <div class="date">
            <form action="{{ route('person.index', ['id' => $user->id]) }}" class="form" method="post">
                @csrf
        ...
        <form action="/export" class="form-csv" method="post">
            @csrf
            <button class="form-csv-button">CSV出力</button>
            <input type="hidden" name="user_id" value="{{ $user->id }}">
            <input type="hidden" name="thisMonth" value="{{ $thisMonth->format('Y/m') }}">
        </form>
    </div>
</div>
@endsection
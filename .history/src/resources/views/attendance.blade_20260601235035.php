        ...
        <div class="item">
            ...
            @foreach($monthDays as $monthDay)
                <tr class="table__inner">
                    ...
                    <td class="table-td">
                        @if($monthDay['apply'])
                        <input type="text" class="table-input" value="{{ substr($monthDay['apply']?->start_time, 0, 5) ?? '' }}" readonly>
                        @elseif($monthDay['work'])
                        <input type="text" class="table-input" value="{{ substr($monthDay['work']?->start_time, 0, 5) ?? '' }}" readonly>
                        @endif
                    </td>
                    <td class="table-td">
                        @if($monthDay['apply'])
                        <input type="text" class="table-input" value="{{ substr($monthDay['apply']?->end_time, 0, 5) ?? '' }}" readonly>
                        @elseif($monthDay['work'])
                        <input type="text" class="table-input" value="{{ substr($monthDay['work']?->end_time, 0, 5) ?? '' }}" readonly>
                        @endif
                    <td class="table-td">
                        @if($monthDay['apply'])
                        <input type="text" class="table-input" value="{{ substr($monthDay['apply']?->duration, 0, 5) ?? '' }}" readonly>
                        @elseif($monthDay['work'])
                    ...
                    <td class="table-td">
                        @if($monthDay['apply'])
                        <input type="text" class="table-input" value="{{ substr($monthDay['apply']?->work_total, 0, 5) ?? '' }}" readonly>
                        @elseif($monthDay['work'])
                    ...
                    <td class="table-td">
                        <div class="link">
                            @if($monthDay['apply'])
                                <a href="{{ route('detail.index', ['id' => $monthDay['apply']->id]) }}" class="link-a">詳細</a>
                            @elseif($monthDay['work'])

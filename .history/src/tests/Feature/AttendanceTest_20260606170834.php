...
class AttendanceTest extends TestCase
{
    ...
    public function test_sub_month()
    {
        ...
        $response = $this->from('/attendance/list')->post('/attendance/list', [
            'year' => $year,
            'month' => $month,
            'action' => 'sub',
        ]);

        $response = get('/attendance/list')->assert('$dayYm');
    }
    ...
    public function test_detail()
    {
        ...
        $response = $this->actingAs($user)->get('/attendance/list')->assertStatus(200);
        $response = $this->get('/attendance/detail/{id}', ['id'=>$attendance->id]);

        $year = Carbon::parse($attendance->date)->isFormat('YYYY年');
        $day = Carbon::parse($attendance->date)->isoFormat('MM月DD日');

        $response->assetSee($yaer);

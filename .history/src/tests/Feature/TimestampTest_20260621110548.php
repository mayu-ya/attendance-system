...
class TimestampTest extends TestCase
{
    ...
    public function test_time_list()
    {
        ...
        $breakStart = Carbon::parse($break->rest_start)->format('H:i');
        $breakEnd = Carbon::parse($break->rest_end)->format('H:i');

        $response = $this->get('/attendance/detail/{id}', ['id'=>$attendance->id])->assertSee($breakStart);
        $response = $this->get('/attendance/detail/{id}', ['id'=>$attendance->id])->assertSee($breakEnd);
    }
    ...
    public function test_end_time_list()
    {
        $user = User::find(1);
        ...
        $attendance = AttendanceRecord::find(19);
        $response = $this->from('/attendance')->post('/working/end', [
            'attendance_record_id' => $attendance->id,
        ]);

        $response = $this->get('/attendance/list')->assertSee($attendance->end_time);
    }
}

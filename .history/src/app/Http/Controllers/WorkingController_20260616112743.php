...
class WorkingController extends Controller
{
    ...
    public function working__end(Request $request)
    {
        ...
        //総労働時間
        $attendances = AttendanceRecord::where('user_id', Autn->id())
        ...
        foreach($attendances as $attendance){
            $apply = Apply::where('status', 'approved')
                            ...
                            ->first();
            if($apply){
                $workTotal += $apply->work_total;
            } else {
                $workTotal += $attendance->work_total;
            }
        }
        ...
        //平均労働時間
        $thisMonth = AttendanceRecord::where('user_id', Autn->id())
                                    ->whereYear('date', $end->year)
                                    ->whereMonth('date', $end->month)
                                    ->count();
        $averageTime = round($total/$thisMonth);
        ...
        if($end->format('H:i') > $leaving) {
            $report->update([
                    'leaving_early' => DB::raw('leaving_early+1'),
                ]);
        }
        ...
        if($totalWork > $today) {
            $report->update([
                'overtime_day' => DB::raw('overtime_day+1'),
            ]);
        }

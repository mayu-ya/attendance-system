...
class AdminWorkingController extends Controller
{
    ...
    public function export(Request $request)
    {
        ...
        $dayYm = carbon::parse($request->thisMonth)->format('Y-m');
        $startMonth = $dayYm->copy()->startOfMonth();
        $endMonth = $dayYm->copy()->endOfMonth();

        $attendances = AttendanceRecord::with('apply', 'user')
                                        ->where('user_id', $userId)
                                        ->whereBetween('date', [$startMonth->toDateString(), $endMonth->toDateStrind()]);

        $csvTitile = [$attendance->user->name . 'さんの勤怠 '];

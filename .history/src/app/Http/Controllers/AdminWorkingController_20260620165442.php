...
class AdminWorkingController extends Controller
{
    ...
    {
        $work = AttendanceRecord::with('user', 'breaks')->where('id', $id)->first();
        $apply = Apply::where('attendance_record_id', $work->id)->first();
        if($apply){
            $apply->rests = Rest::where('apply_id', $apply->id)->get();
        }

        return view('admin.detail', compact('work', 'apply'));
    }
    ...
    public function export(Request $request)
    {
        ...
        $response = new StreamedResponse(function () use($dayYm, $startMonth, $attendances, $csvAttendance) {
            $handle = fopen('php://output', 'w');
            fwrite($handle, pack('C*', 0xEF, 0xBB, 0xBF));
            
            $record = $attendances->first();
            fputcsv($handle, [$record->user->name . 'さんの勤怠 ']);
            ...
            for ($i = 0; $i < $daysMonth; $i++) {
                ...
                $work = $attendances->get($currentDate);

                if($work){
                    $startTime = Carbon::parse($work->start_time)->format('H:i');
                    $endTime = Carbon::parse($work->end_time)->format('H:i');
                    $duration = Carbon::parse($work->duration)->format('H:i');
                    $workTotal = Carbon::parse($work->work_total)->format('H:i');
                } else {
                    $startTime = '';
                    $endTime = '';
                    $duration = '';

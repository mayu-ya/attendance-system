    ...
    public function working__end(Request $request)
    {
        ...
        foreach($attendances as $attendance){
            $apply = Apply::where('status', 'approved')
                            ...
                            ->first();
            if($apply){
                $time = $apply->work_total;
            } else {
                $time = $attendance->work_total;
            }
            $timeTotal = Carbon::createFromFormat('H:i:s',$time);
            sscanf($timeTotal, '%d:%d:%d', $hours, $minutes, $second);
            $workTotal += ($hours * 60) + ($minutes * 60) + $second;
        }

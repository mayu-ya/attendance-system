...
class AdminApplyController extends Controller
{
    ...
    public function request($attendance_correct_request_id)
    {
        ...
        $apply->start_time = Corbon::parse($apply->end_time)->format('H:i');

        foreach($apply->rests as $rest){
            $apply->rest_start = Corbon::parse($rest->rest_start)->format('H:i');
            $apply->rest_end = Corbon::parse($rest->rest_end)->format('H:i');
        }
